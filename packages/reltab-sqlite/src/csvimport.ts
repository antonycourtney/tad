 ...
 */
const guessColumnType = numREs => (cg: ColumnType | null | undefined, cs: string | null | undefined): ColumnType | null | undefined => {
  if (cg === 'text') {
...
const prepValue = (ct: ColumnType | null | undefined, vs: string | null | undefined, isEuroFormat: boolean): string | null | undefined => {
    ...
    const csvStream =
      csv(csvOptions)
        ...
        .on('end', () => {
          ...
          const columnTypes = colTypes.map((ct) => (ct == null ? 'text' : (ct as any)))
 ...
 */
const importData = async (db: sqlite3.Database, md: FileMetadata, pathname: string,
  delimiter: string, options: ImportOpts): Promise<FileMetadata> => {
  try {
    const hasHeaderRow = !options.noHeaderRow
...
const dbImport: (
  ...
  fnm: string,
  tnm: string,
  opts: ImportOpts) => Promise<ImportResult> = tp.promisify((db, fnm, tnm, opts, cb) => db.import(fnm, tnm, opts, cb));
