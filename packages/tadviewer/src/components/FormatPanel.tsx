...
export class FormatPanel extends React.Component {
  state: Object;
  ...
  constructor(props: any) {
    ...
    const firstCol =
      schema.columns && schema.columns.length > 0 ? schema.columns[0] : null;
    this.state = {
      formatKind: "default",
      colType: "text",
  ...
  renderFormatPanel() {
    ...
    const updater = this.props.stateRefUpdater; // return appropriate subpanel component type for given
    // column type:
    ...
    if (this.state.formatKind === "default") {
      ...
      changeHandler = fopts =>
        actions.setDefaultFormatOptions(colType, fopts, updater);
    } else {
      ...
      changeHandler = fopts =>
        actions.setColumnFormatOptions(targetColumn, fopts, updater);
    }

    const PanelComponent = subPanelComponentForType(colType);
    const formatPanel = (
      <PanelComponent value={currentOpts} onChange={changeHandler} />
    );
    return formatPanel;
  }
