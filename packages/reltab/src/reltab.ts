...
const escRegEx = /[\0\n\r\b\t'"\x1a]/g;
export const sqlEscapeMbString = (inStr: string | undefined | null): string | undefined | null => {
...
export type BinRelOp = "EQ" | "NEQ" | "GT" | "GE" | "LT" | "LE" | "BEGINS" | "NOTBEGINS" | "ENDS" | "NOTENDS" | "CONTAINS" | "NOTCONTAINS" | "IN" | "NOTIN";
export type UnaryRelOp = "ISNULL" | "NOTNULL";
export type RelOp = UnaryRelOp | BinRelOp;
const textOnlyBinaryOps: RelOp[] = ['IN', 'NOTIN', 'BEGINS', 'NOTBEGINS', 'ENDS', 'NOTENDS', 'CONTAINS', 'NOTCONTAINS'];
const textOnlyOpsSet = new Set(textOnlyBinaryOps);
const textNegBinaryOps: RelOp[] = ['NOTIN', 'NOTBEGINS', 'NOTENDS', 'NOTCONTAINS'];
...
type QueryOp = "table" | "project" | "filter" | "groupBy" | "mapColumns" | "mapColumnsByIndex" | "concat" | "sort" | "extend" | "join";
...
export type Scalar = number | undefined | null | string | undefined | null | boolean | undefined | null;
export type Row = {
...
const numericAggFns: AggFn[] = ['avg', 'count', 'min', 'max', 'sum', 'uniq', 'null'];
...
export class QueryExp {
  ...
  mapColumnsByIndex(cmap: {
    [colIndex: string]: ColumnMapInfo;
  }): QueryExp {
  ...
  toSql(tableMap: TableInfoMap, offset: number = -1, limit: number = -1): string {
...
const joinGetSchema = (tableMap: TableInfoMap, query: QueryExp): Schema => {
  ...
  const rhsCols = _.difference(rhsSchema.columns, _.concat(on, lhsSchema.columns));
...
const quoteCol = cid => '"' + cid + '"'; // Gather map by column id of SQLSelectColExp in a SQLSelectAST


const selectColsMap = (selExp: SQLSelectAST): {
...
const groupByQueryToSql = (tableMap: TableInfoMap, query: QueryExp): SQLQueryAST => {
  ...
  const genAgg = (aggStr, qcid) => aggStr + '(' + qcid + ')'; // Get the aggregation expressions for each aggCol:
  ...
  if (sqsql.selectStmts.length === 1 && _.every(subSel.selectCols, sc => typeof sc === 'string') && subSel.where.length === 0 && subSel.groupBy.length === 0 && subSel.orderBy.length === 0) {
    retSel = _.defaults({
      selectCols,
      groupBy: cols
    }, subSel);
...
const filterQueryToSql = (tableMap: TableInfoMap, query: QueryExp): SQLQueryAST => {
  ...
  if (sqsql.selectStmts.length === 1 && subSel.where.length === 0 && subSel.groupBy.length === 0) {
    retSel = _.defaults({
      where: whereStr
    }, subSel);
  } else {
...
const sortQueryToSql = (tableMap: TableInfoMap, query: QueryExp): SQLQueryAST => {
  const sqsql = queryToSql(tableMap, query.tableArgs[0]);
  const orderBy = query.valArgs[0].map(([col, asc]) => ({
    col,
    asc
...
export class Schema {
  ...
  sortedColumns(): Array<string> {
    ...
    if (sc == null) {
      ...
      sc.sort((cid1, cid2) => this.displayName(cid1).localeCompare(this.displayName(cid2)));
